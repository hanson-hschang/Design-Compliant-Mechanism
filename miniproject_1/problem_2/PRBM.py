...
def run_force_length_retaion(plot=False, **kwargs):
    model = build_model(**kwargs)
    if plot:
        fig, ax = plt.subplots()
        model.plot(ax, color='C0', linestyle='--')
    displacements = np.linspace(0, 30, 21)
    applied_force = np.zeros(displacements.shape)
    for i, displacement in enumerate(displacements[1:]):
        model.boundary_constraints[4][2] = -displacement
        model.deform()
        applied_force[i+1] = np.abs(model.reaction_load[4][2])
        if plot:
    ...
    return displacements, applied_force
...
def main():
    # fig, ax = plt.subplots()
    # for l1 in [25, 30, 35, 40]:
    #     displacements, applied_force = run_force_length_retaion(l1=l1)
    #     ax = plot_force_length_relation(ax, displacements, applied_force)
    # fig.tight_layout()

    # fig, ax = plt.subplots()
    # for theta2 in [20, 19, 18, 17, 16, 15, 14, 13, 12, 11, 10,]:
    #     displacements, applied_force = run_force_length_retaion(theta2=theta2)
    #     ax = plot_force_length_relation(ax, displacements, applied_force)
    # fig.tight_layout()

    # fig, ax = plt.subplots()
    # for h1 in [3, 3.5, 4, 4.5, 5, 5.5, 6, 6.5, 7,]:
    #     displacements, applied_force = run_force_length_retaion(h1=h1)
    #     ax = plot_force_length_relation(ax, displacements, applied_force)
    # fig.tight_layout()

    # fig, ax = plt.subplots()
    # for h2 in [0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4]:
    #     displacements, applied_force = run_force_length_retaion(h2=h2)
    #     ax = plot_force_length_relation(ax, displacements, applied_force)
    # fig.tight_layout()

    # fig, ax = plt.subplots()
    # for youngs_modulus in [2, 20, 200, 2_000, 20_000, 200_000, 2_000_000]:
    #     displacements, applied_force = run_force_length_retaion(
    #         youngs_modulus=[2000, youngs_modulus]
    #     )
    #     ax = plot_force_length_relation(ax, displacements, applied_force)
    # fig.tight_layout()

    fig, ax = plt.subplots()
    displacements, applied_force = run_force_length_retaion(plot=True)
    ax = plot_force_length_relation(ax, displacements, applied_force)
    displacements, applied_force = run_force_length_retaion(
        plot=True,
        l1=35,
        ...
        h1=5,
        h2=2,
        youngs_modulus=[2_000, 2_000_000]
    )
    ax = plot_force_length_relation(ax, displacements, applied_force)
    fig.tight_layout()
